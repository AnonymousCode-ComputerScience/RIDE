...
PROJECT_ABSOLUTE_PATH = os.path.dirname((os.path.dirname(os.path.dirname((os.path.dirname(os.path.abspath(__file__)))))))

# test_data = load_dataset("re-align/just-eval-instruct", split="test")
test_data = load_dataset("json", data_files=PROJECT_ABSOLUTE_PATH+"/run_scripts/just_eval/test_all_with_tags.jsonl", split="train")
...
config_file = PROJECT_ABSOLUTE_PATH+"/just-eval/leaderboard/configs_gpt4o.json"
with open(config_file) as f:
    # print(f.name)
    configs = json.load(f)
if lite:
    filter_file = "just-eval/leaderboard/filters.json"
...
table = []
for item in configs:
    with open(PROJECT_ABSOLUTE_PATH+'/'+item["eval_result_general"]) as f:
    ...
    for eval_item in eval_data:
        if eval_item["parsed_result"] is None:
            print('Empty item: {}'.format(eval_item))
            continue
        _id = eval_item["id"]
    ...
    if show_key in ["aspect", "source"]:
        with open(PROJECT_ABSOLUTE_PATH+'/'+item["eval_result_safety"]) as f:
        ...
        for eval_item in eval_data_safety:
            ...
            try:
                ...
                print("Missing keys in nested dictionary:", eval_item)
            # lens.append(len(eval_item["output_cand"].split()))
    dim_score_avg = {} 
