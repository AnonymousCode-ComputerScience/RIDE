...
def get_random_subset(dataset, num_samples=100):
    """
    Randomly select a subset of the dataset with a given number of samples.
    """
    if num_samples > len(dataset):
        raise ValueError("Number of samples requested exceeds the size of the dataset.")
    random.seed(RANDOM_SEED)
    select_elements = random.sample(range(len(dataset)), num_samples)
    subset = dataset.select(select_elements)
    return subset
...
def load_eval_data(args, data_name=None, model_name=None):
    ...
    if data_name == "alpaca_eval":
        ...
        if subset_num is not None:
            dataset = get_random_subset(dataset=dataset, num_samples=subset_num)
        # print('dataset is:')
        # print(dataset)
        metadata = {"dataset": []}
    elif data_name == "just_eval":
        # dataset = load_dataset("re-align/just-eval-instruct", split="test")
        dataset = load_dataset("json", data_files="run_scripts/just_eval/test_all_with_tags.jsonl", split="train")
        print('dataset is:')
        print(dataset)
        if subset_num is not None:
            dataset = get_random_subset(dataset=dataset, num_samples=subset_num)
...
def save_outputs(args, id_strs, outputs, chat_history, metadata, model_inputs, filepath):
    ...
    if args.data_name == "alpaca_eval":
        for ind in range(len(outputs)):
            ...
            output_item["generator"] = f"{args.model_name}-{args.urial}" if args.urial else args.model_name
